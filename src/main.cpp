...
// Factory function to create spheres with specified properties
Sphere createSphere(const glm::vec3& centerPosition, float radius, const glm::vec3& color,
    const glm::vec3& specular = glm::vec3(0.0f), float shininess = 1.0f, float transparency = 1.0f)
{
    return Sphere {
        .center = centerPosition, // Center position of the sphere
        .radius = radius, // Radius of the sphere
        .material = {
            .kd = color, // Diffuse color
            .ks = specular, // Specular color
            .shininess = shininess, // Shininess value
...
void drawing()
{
    ...
    std::vector<Ray> drawRays = take_rays();
    ...
    if (drawRays.size() > 0) 
        for (auto r : drawRays) 
        {
            if (cnt == 0)
                break;
            cnt--;
            drawRay(r, glm::vec3(1.0f, 0.0f, 1.0f));
        }
}
        ...
        while (!window.shouldClose()) {
            ...
            if (ImGui::CollapsingHeader("Extra Features")) {
                ...
                if (config.features.extra.enableBloomEffect) {
                    ImGui::Indent();
                    uint64_t minFilterSize = 1;
                    uint64_t maxFilterSize = 9;
                    ImGui::Checkbox("GrayScale image", &config.features.extra.enableGrayScale);
                    if (config.features.extra.enableGrayScale) {
                        if (ImGui::Button("Choose grayscale filter")) {
                            nfdchar_t* inputPath = nullptr;
                            const nfdresult_t result = NFD_OpenDialog("bmp,png,jpg,jpeg", nullptr, &inputPath);
                            if (result == NFD_OKAY) {
                                std::filesystem::path inPath { inputPath };
                                free(inputPath);
                                auto im = std::make_shared<Image>(inPath);
                                if(im->width % 2 == 0 || im->height % 2 == 0) {
                                    ImGui::OpenPopup("Wrong bloom filter dimensions");
                                } else {
                                    config.features.extra.bloomGrayscaleFilter = im;
                                }
                            }
                        }
                    } else {
                        ImGui::SliderScalar("Gaussian filter size", ImGuiDataType_U64, &config.features.extra.bloomFilterSize, &minFilterSize, &maxFilterSize);
                    }
                    if(ImGui::BeginPopupModal("Wrong bloom filter dimensions")) {
                        ImGui::Text("Bloom filter must have odd width and height");
                        if(ImGui::Button("Ok")) {
                    ...
                    float min = 0.0f;
                    float max = 1.0f;
                    if(ImGui::SliderScalar("Minimum threshold", ImGuiDataType_Float, &config.features.extra.thresholdMin, &min, &max)) {
                        config.features.extra.thresholdMin = glm::min(config.features.extra.thresholdMin, config.features.extra.thresholdMax);
                    }
                    if(ImGui::SliderScalar("Maximum threshold", ImGuiDataType_Float, &config.features.extra.thresholdMax, &min, &max)) {
                        config.features.extra.thresholdMax = glm::max(config.features.extra.thresholdMin, config.features.extra.thresholdMax);
                    }
                    ...
                    ImGui::Combo("Masking function type", reinterpret_cast<int*>(&config.features.extra.bloomFunctionType), items2.data(), int(items2.size()));

                    constexpr std::array items {
                        "Normal (image with bloom)",
                        "Bloom contribution",
                        "Brightness mask"
                    };
                    ImGui::Combo("Bloom render type", reinterpret_cast<int*>(&config.features.extra.bloomRenderType), items.data(), int(items.size()));
                    ImGui::Unindent();
                }
                ...
                if (config.features.extra.enableDepthOfField) {
                    ...
                    float mini = 0.00f, maxi = 0.5f;
                    ImGui::SliderScalar("Lens Radius", ImGuiDataType_Float, &config.features.extra.lr, &mini, &maxi);
                    ImGui::SliderFloat("Focal Distance", &config.features.extra.fd, 1.0f, 10.0f, "%3.0f");
                    ImGui::SliderFloat("Focal Length", &config.features.extra.fl, 0.1f, 25.0f, "%1.0f");
                ...
                if (config.features.extra.enableEnvironmentMap) {
                    float minCubeSize = 1.0f, maxCubeSize = 50.0f;
                    ImGui::Indent();
                    ImGui::Checkbox("Use sphere map", &config.features.extra.useSphereMap);
                    if(!config.features.extra.useSphereMap) {
                        ImGui::SliderScalar("Cube map size", ImGuiDataType_Float, &config.features.extra.cubeMapSize, &minCubeSize, &maxCubeSize);
                    }
                    ImGui::Unindent();
                }
            ...
            if (ImGui::Button("Focal Points")) {
                std::vector<glm::vec3> fp_points = take_fp();
                std::vector<glm::vec3> lens_points = take_lens();
                std::vector<glm::vec3> image_points = take_image();
                int stop;

                // draw focal plane points
                stop = 6000;
                for (glm::vec3 point : fp_points) 
                {
                    if (stop == 0)
                        break;
                    stop--;
                    Sphere simpleSphere = createSphere(glm::vec3(0.0f, 1.0f, 2.0f), 0.2f, glm::vec3(1.0f, 0.0f, 0.0f));
                    scene.spheres.emplace_back(simpleSphere);
                }
                ...
                // draw lens points
                stop = 2000;
                for (glm::vec3 point : lens_points) {
                    if (stop == 0)
                        break;
                    stop--;
                    Sphere glossySphere = createSphere(glm::vec3(2.0f, 3.0f, 4.0f), 0.2f, glm::vec3(0.0f, 0.5f, 1.0f), glm::vec3(0.3f), 10.0f, 0.5f);
                    scene.spheres.emplace_back(glossySphere);
                }
                ...
                // draw image points
                stop = 5000;
                for (glm::vec3 point : image_points) {
                    if (stop == 0)
                        break;
                    stop--;
                    Sphere reflectiveTransparentSphere = createSphere(glm::vec3(1.0f, 1.0f, 1.0f), 0.2f, glm::vec3(0.8f, 0.8f, 0.8f), glm::vec3(1.0f), 50.0f, 1.0f);
                    scene.spheres.emplace_back(reflectiveTransparentSphere);
                }
            }
