...
Scene loadScenePrebuilt(SceneType type, const std::filesystem::path& dataDir)
{
    ...
    case EnvironmentMapDebug: {
        scene.spheres.push_back(Sphere { glm::vec3(0.0f, 0.0f, 0.0f), 0.5f, Material {.kd = glm::vec3(1.0f, 1.0f, 1.0f), .ks = glm::vec3(1.0f, 1.0f, 1.0f) } });
        scene.lights.emplace_back(PointLight { glm::vec3(0.0f, 5.0f, 5.0f), glm::vec3(.3f, .3f, .3f) });
    } break;
    case Custom: {
    ...
    scene.cubeMap = {
        // those files are parts of a public domain image downloaded from Wikimedia: https://commons.wikimedia.org/wiki/File:Cube_map_level.png
        std::make_shared<Image>(dataDir / "cube_map_0.png"),
        std::make_shared<Image>(dataDir / "cube_map_1.png"),
        std::make_shared<Image>(dataDir / "cube_map_2.png"),
        std::make_shared<Image>(dataDir / "cube_map_3.png"),
        std::make_shared<Image>(dataDir / "cube_map_4.png"),
        std::make_shared<Image>(dataDir / "cube_map_5.png")
    };

    // this is also a public domain image from Wikimedia: https://commons.wikimedia.org/wiki/File:Mercator-projection.jpg
    scene.sphereMap = std::make_shared<Image>(dataDir / "sphere_map.jpg");
