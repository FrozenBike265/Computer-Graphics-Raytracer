...
struct ExtraFeatures {
    ...
    // Some parameters for Dof
    float lr = 0.01f; // radius of lens
    float fd = 3.0f; // focal distance
    float fl = 1.0f; // focal length

    // Parameters for glossy reflection
    uint32_t numGlossySamples = 1;

    float bloomThreshold = 0.9f;
    uint64_t bloomFilterSize = 5;
    BloomRenderType bloomRenderType = NORMAL;

    bool enableGrayScale = false;
    std::shared_ptr<Image> bloomGrayscaleFilter;

    float thresholdMin = 0.9f;
    float thresholdMax = 1.0f;
    BloomFunctionType bloomFunctionType = CONSTANT;

    float cubeMapSize = 10.0f;
